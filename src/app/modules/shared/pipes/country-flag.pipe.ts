...
const countries = [
    ...
    {
        code: 'xk',
        name: 'UNSC resolution 1244-administered Kosovo',
        path: 'kosovo',
    },
    {
        code: 'kw',
...
export class CountryFlagPipe implements PipeTransform {
    transform(value: string | string[]): string {
        if (typeof value === 'string') {
            return this.toFlagSpan(value);
        } else {
            return value.map(val => this.toFlagSpan(val)).join('');
        }
    }
    toFlagSpan(value: string): string {
        if (!value) return '';
