...
@Component({
    selector: 'quan-ly-nguoi-dung-dashboard',
    template: `
...
export class QuanLyNguoiDungContainer implements OnInit, OnDestroy {
    pageSize = 10;

    @ViewChild(DanhSachNguoiDung, { static: true })
    listUserForm: DanhSachNguoiDung;
    ...
    constructor(private service: QuanLyNguoiDungService, private dialog: MatDialog, private spinner: NgxSpinnerService,) { }
    ...
    ngOnInit() {
        combineLatest([this.paginateUsers$, this.filter$, this.sorter$])
            .pipe(
                ...
                switchMap(([pagination, filter, sorter]) => {
                    ...
                    this.spinner.show();
                    return this.service.paginteAdmins(pagination, filter, orderBy);
                }),
        ...
        this.inactivateAdmin$
            .pipe(
                delay(0),
                exhaustMap(res => this.service.deleteEmployee(res.userId, this.pageSize)),
                takeUntil(this.unsubscribe$),
            )
            .subscribe();
        this.inactivateUser$
            .pipe(
                delay(0),
                exhaustMap(res => this.service.inactivateUser(res.userId, res.status, this.pageSize)),
                takeUntil(this.unsubscribe$),
            )
    ...
    openDialog(data: { username: string; userId: string }) {
        ...
        dialogRef
            ...
            .subscribe(async result => {
                if (result) {
                    this.spinner.show();
                    const deleteResult = await this.service.deleteEmployee(userId, this.pageSize);
                    if (deleteResult) {
                        this.spinner.hide();
        ...
        dialogRef
            ...
            .pipe(
                switchMap((res) => {
                    if (res) {
                        this.spinner.show();
                        return this.service.inactivateUser(data.userId, data.status, this.pageSize);
                    }
                    return of(undefined);
                }),
                takeUntil(this.unsubscribe$)
