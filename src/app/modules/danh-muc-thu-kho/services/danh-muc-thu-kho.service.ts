...
import { ThuKhoModel } from '../types';

// const TRANSACTION_NAME = environment.elasticAPM.transactionName;

@Injectable()
export class DanhMucThuKhoService {
    urlDefault = this.apiConstant.endpoint;
    urlByService = `${this.urlDefault}/qlnv-category/dmuc-thukho`;
    ...
    create(input: ThuKhoModel, options: PaginateOptions){
        const headers = new HttpHeaders();
        return this.httpClient
            .post<any>(
    ...
    update(input: ThuKhoModel, options: PaginateOptions){
        const headers = new HttpHeaders();
        return this.httpClient
            .post<any>(
