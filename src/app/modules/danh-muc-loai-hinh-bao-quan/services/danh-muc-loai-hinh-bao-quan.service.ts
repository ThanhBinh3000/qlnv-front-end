...
import { LoaiHinhBaoQuanModel } from '../types';

@Injectable()
export class DanhMucLoaiHinhBaoQuanService {
    urlDefault = this.apiConstant.endpoint;
    urlByService = `${this.urlDefault}/qlnv-category/dmuc-lhinh-bquan`;
    ...
    private paginateByUserRole(
        ...
        optionSearch?: LoaiHinhBaoQuanModel
    ) {
        let params;
    ...
    delete(id: string, pageSize: number): Promise<any> {
        const headers = new HttpHeaders();
        return this.httpClient
            .get<any>(`${this.urlByService}/delete/${id}`, { headers })
            .pipe(
                tap(() => {
                    this.errorsSub$.next([]);
                }),
    ...
    create(input: LoaiHinhBaoQuanModel, options: PaginateOptions){
        const headers = new HttpHeaders();
        return this.httpClient
            .post<any>(
                `${this.urlByService}/create`,
                input,
                { headers },
            )
            .pipe(
    ...
    update(input: LoaiHinhBaoQuanModel, options: PaginateOptions){
        const headers = new HttpHeaders();
        return this.httpClient
            .post<any>(
                `${this.urlByService}/update`,
                input,
                { headers },
            )
            .pipe(
