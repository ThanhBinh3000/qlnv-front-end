...
import { LoaiHinhBaoQuanDataModel } from '../..';
import { DEFAULT_OPTION_SEARCH } from '../../constants';
import { DanhMucLoaiHinhBaoQuanService } from '../../services/danh-muc-loai-hinh-bao-quan.service';
import { ThemSuaDanhMucLoaiHinhBaoQuan } from '../them-sua-danh-muc-loai-hinh-bao-quan/them-sua-danh-muc-loai-hinh-bao-quan.component';

@Component({
    selector: 'danh-sach-danh-muc-loai-hinh-bao-quan',
    templateUrl: './danh-sach-danh-muc-loai-hinh-bao-quan.component.html',
    styleUrls: ['./danh-sach-danh-muc-loai-hinh-bao-quan.component.scss'],
    changeDetection: ChangeDetectionStrategy.OnPush,
})
export class DanhSachDanhMucLoaiHinhBaoQuan implements OnInit, OnDestroy, OnChanges, AfterViewInit {
    @Input()
    userCollection: any;
    ...
    optionSearch: LoaiHinhBaoQuanDataModel = DEFAULT_OPTION_SEARCH;
    ...
    constructor(
        ...
        private service: DanhMucLoaiHinhBaoQuanService,
    ...
    delete(event: any,element: any) {
        if (element && element.id > 0) {
            let dialogRef = this.dialog.open(ConfirmCancelDialog, {
                width: '546px',
                data: {
                    title: `Xóa danh mục loại hình bảo quản`,
                    subTitle: `Bạn có chắc chắn muốn xóa danh mục loại hình bảo quản?`,
                    message: `Khi xóa dữ liệu, các dữ liệu liên quan đến loại hình bảo quản "${element.tenLhinh}" sẽ bị xóa đi.`,
                        ...
                        if (deleteResult) {
                            ...
                            this.dialog.open(ConfirmationDialog, {
                                ...
                                data: {
                                    ...
                                    message: `Danh mục loại hình bảo quản đã được xóa thành công.`,
    ...
    commonFunc(element: any, isView: boolean) {
        const termDialog = this.dialog.open(ThemSuaDanhMucLoaiHinhBaoQuan, {
            width: '450px',
            data: {
                title: isView ? 'Thông tin loại hình bảo quản' : 'Cập nhật loại hình bảo quản',
                isView: isView,
                id: element.id,
    ...
    create() {
        const termDialog = this.dialog.open(ThemSuaDanhMucLoaiHinhBaoQuan, {
            width: '450px',
            data: {
                title: 'Thêm mới loại hình bảo quản',
                isView: false,
                id: 0,
                maLhinh: null,
                tenLhinh: '',
    ...
    initDatatable(data: any, count: number, startAtPage?: { pageIndex: number }) {
        ...
        this.config = {
            data,
            tableName: 'pra-users',
            filterKeys: ['maLhinh','tenLhinh', 'ghiChu', 'trangThai'],
            hideFilter: false,
            columns: [
                {
                    text: 'Mã loại hình bảo quản',
                    label: 'Mã loại hình bảo quản',
                    fieldName: 'maLhinh',
                    style: { flex: 2 },
                    sortable: false,
                },
                {
                    text: 'Tên loại hình bảo quản',
                    label: 'Tên loại hình bảo quản',
                    fieldName: 'tenLhinh',
                    style: { flex: 2 },
                    sortable: false,
                },
