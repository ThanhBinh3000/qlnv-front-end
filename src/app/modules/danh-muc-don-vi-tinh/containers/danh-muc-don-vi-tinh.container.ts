...
@Component({
    selector: 'danh-muc-don-vi-tinh-dashboard',
    template: `
        <div class="grid-container1 main-content">
            <danh-sach-danh-muc-don-vi-tinh
...
export class DanhMucDonViTinhContainer implements OnInit, OnDestroy {
    pageSize = 10;

    @ViewChild(DanhSachDanhMucDonViTinh, { static: true })
    listUserForm: DanhSachDanhMucDonViTinh;

    private unsubscribe$ = new Subject();
    paginateUsers$ = new BehaviorSubject<PaginateOptions>({ pageIndex: 0, pageSize: this.pageSize });
    userCollection$ = this.service.listDonViTinh$;
    errors$ = this.service.errors$;

    constructor(private service: DanhMucDonViTinhService, private dialog: MatDialog, private spinner: NgxSpinnerService,) { }
