...
import { PhuongThucDauThauModel } from '../types';

// const TRANSACTION_NAME = environment.elasticAPM.transactionName;

@Injectable()
export class DanhMucPhuongThucDauThauService {
    urlDefault = this.apiConstant.endpoint;
    urlByService = `${this.urlDefault}/qlnv-category/dmuc-pthuc-dthau`;
    ...
    private paginateByUserRole(
        ...
        optionSearch?: PhuongThucDauThauModel
    ) {
        let params;
    ...
    create(input: PhuongThucDauThauModel, options: PaginateOptions){
        const headers = new HttpHeaders();
        return this.httpClient
            .post<any>(
    ...
    update(input: PhuongThucDauThauModel, options: PaginateOptions){
        const headers = new HttpHeaders();
        return this.httpClient
            .post<any>(
