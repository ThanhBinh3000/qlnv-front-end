...
import { QuocGiaSanXuatModel } from '../types';

// const TRANSACTION_NAME = environment.elasticAPM.transactionName;

@Injectable()
export class DanhMucQuocGiaSanXuatService {
    urlDefault = this.apiConstant.endpoint;
    urlByService = `${this.urlDefault}/qlnv-category/dmuc-quocgia`;
    ...
    private paginateByUserRole(
        ...
        optionSearch?: QuocGiaSanXuatModel
    ) {
        let params;
    ...
    create(input: QuocGiaSanXuatModel, options: PaginateOptions){
        const headers = new HttpHeaders();
        return this.httpClient
            .post<any>(
    ...
    update(input: QuocGiaSanXuatModel, options: PaginateOptions){
        const headers = new HttpHeaders();
        return this.httpClient
            .post<any>(
