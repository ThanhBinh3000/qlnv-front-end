...
import { DonViLienQuanModel } from '../types';

// const TRANSACTION_NAME = environment.elasticAPM.transactionName;

@Injectable()
export class DanhMucDonViLienQuanService {
    urlDefault = this.apiConstant.endpoint;
    urlByService = `${this.urlDefault}/qlnv-category/dmuc-dvi-lquan`;
    ...
    private paginateByUserRole(
        ...
        optionSearch?: DonViLienQuanModel
    ) {
        let params;
    ...
    create(input: DonViLienQuanModel, options: PaginateOptions){
        const headers = new HttpHeaders();
        return this.httpClient
            .post<any>(
    ...
    update(input: DonViLienQuanModel, options: PaginateOptions){
        const headers = new HttpHeaders();
        return this.httpClient
            .post<any>(
