...
@Component({
  ...
  templateUrl: './tong-hop-de-xuat-nhu-cau-bao-hiem-cuc.component.html',
  styleUrls: ['./tong-hop-de-xuat-nhu-cau-bao-hiem-cuc.component.scss']
})
export class TongHopDeXuatNhuCauBaoHiemCucComponent implements OnInit {
    ...
    if (dataDelete && dataDelete.length > 0) {
      this.modal.confirm({
        ...
        nzOnOk: async () => {
          this.spinner.show();
          try {
            let res = await this.bienBanChuanBiKhoService.delete({ ids: dataDelete });
            if (res.msg == MESSAGE.SUCCESS) {
