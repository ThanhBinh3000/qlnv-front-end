...
export class ThongTinDeNghiCapPhiBoNganhComponent implements OnInit {
  ...
  addRow() {
    ...
    this.rowEdit.ct2s = [...this.rowEdit.ct2s, item];
    this.clearItemRow();
  }
  ...
  async loadChiTiet(id) {
    if (id > 0) {
      ...
      if (res.msg == MESSAGE.SUCCESS) {
        if (res.data) {
          this.deNghiCapPhiBoNganh = res.data;
          this.initForm();
          /* if (this.deNghiCapPhiBoNganh.fileDinhKems) {
            this.deNghiCapPhiBoNganh = this.khBanDauGia.fileDinhKems;
          }
  ...
  initForm() {
    this.formData = this.fb.group({
      nam: [
        {
          value: this.deNghiCapPhiBoNganh ? this.deNghiCapPhiBoNganh.nam : null,
