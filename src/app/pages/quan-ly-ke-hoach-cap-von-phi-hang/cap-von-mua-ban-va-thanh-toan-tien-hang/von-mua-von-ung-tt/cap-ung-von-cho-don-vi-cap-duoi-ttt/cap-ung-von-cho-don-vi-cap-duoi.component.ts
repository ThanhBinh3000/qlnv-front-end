...
export class CapUngVonChoDonViCapDuoiComponent implements OnInit {
    ...
    await this.capVonMuaBanTtthService.ctietCapVon(this.id).toPromise().then(
      async (data) => {
        if (data.statusCode == 0) {
          ...
          this.lstCtietBcao = data.data.capUngVonDviCapDuoi;
  ...
  async save() {
    ...
    this.lstCtietBcao.forEach(item => {
      if (item.vongUng > MONEY_LIMIT || item.vonCap > MONEY_LIMIT || item.tong > MONEY_LIMIT) {
    ...
    const request = JSON.parse(JSON.stringify({
      ...
      capUngVonDviCapDuoi: lstCtietBcaoTemp,
  ...
  changeModel(id: string): void {
    this.editCache[id].data.tong = this.editCache[id].data.vongUng + this.editCache[id].data.vonCap;
    this.editCache[id].data.capUngVonDviCapDuoiLuyKe[0].tongLuyKe = this.editCache[id].data.capUngVonDviCapDuoiLuyKe[0].vonUngLuyKe + this.editCache[id].data.capUngVonDviCapDuoiLuyKe[0].vonCapLuyKe;
    // this.lstCtietBcao.forEach(item => {
    //   this.editCache[id].item.capUngVonDviCapDuoiLuyKe
    // }console.log('item', item))
  }

  changeDate(id: string) {
    this.editCache[id].data.ngayLap = this.datePipe.transform(this.editCache[id].data.ngayLap, Utils.FORMAT_DATE_STR);
