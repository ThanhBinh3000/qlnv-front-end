...
export class TienThuaComponent implements OnInit {
                ...
                async (data) => {
                    if (data.statusCode == 0) {
                        ...
                        this.router.navigate([
                            MAIN_ROUTE_CAPVON + '/' + CAP_VON_MUA_BAN + '/tien-thua/0/'
                            + data.data.id,
                        ]);
                    } else {
                        this.notification.error(MESSAGE.ERROR, data?.msg);
    ...
    close() {
        ...
        if (this.statusBtnParent) {
            this.router.navigate([
                MAIN_ROUTE_CAPVON + '/' + CAP_VON_MUA_BAN + '/danh-sach-nop-tien-thua/' + this.loai
            ]);
        } else {
            this.router.navigate([
                MAIN_ROUTE_CAPVON + '/' + CAP_VON_MUA_BAN + '/danh-sach-ghi-nhan-tien-von-thua/' + this.loai
            ]);
        }
    }
