...
@Component({
	...
	templateUrl: './de-nghi-cap-von-mua-thoc-gao-muoi-theo-don-gia-mua.component.html',
	styleUrls: ['./de-nghi-cap-von-mua-thoc-gao-muoi-theo-don-gia-mua.component.scss',
	],
})
...
export class DeNghiCapVonMuaThocGaoMuoiTheoDonGiaMuaComponent implements OnInit {
	...
	getStatusButton() {
		...
		this.status = Utils.statusSave.includes(this.baoCao.trangThai) && this.userService.isAccessPermisson(Roles.CVNC.EDIT_DN_MLT) && !this.userService.isTongCuc();
	...
	changeModel(id: string) {
		this.editCache[id].data.gtriThucHien = Operator.mul(this.editCache[id].data.slThucHien, this.editCache[id].data.soConDuocCap);
		...
		this.editCache[id].data.vonDnghiCapLanNay = Operator.sum([this.editCache[id].data.gtriThucHien, -this.editCache[id].data.tongVonVaDtDaCap]);
	}
