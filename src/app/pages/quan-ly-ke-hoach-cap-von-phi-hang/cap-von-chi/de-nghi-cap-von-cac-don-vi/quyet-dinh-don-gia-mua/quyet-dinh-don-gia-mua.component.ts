...
@Component({
    selector: 'app-quyet-dinh-don-gia-mua',
    templateUrl: './quyet-dinh-don-gia-mua.component.html',
    styleUrls: ['../de-nghi-cap-con-cac-don-vi.component.scss'],
})
export class QuyetDinhDonGiaMuaComponent implements OnInit {
    @Input() dataInfo;
    ...
    getStatusButton() {
        ...
        const checkSave = this.baoCao.maLoai == Cvnc.CAP_VON ? this.userService.isAccessPermisson(Roles.CVNC.EDIT_CV) : this.userService.isAccessPermisson(Roles.CVNC.EDIT_DN);
        const checkSubmit = this.baoCao.maLoai == Cvnc.CAP_VON ? this.userService.isAccessPermisson(Roles.CVNC.SUBMIT_CV) : this.userService.isAccessPermisson(Roles.CVNC.SUBMIT_DN);
        const checkPass = this.baoCao.maLoai == Cvnc.CAP_VON ? this.userService.isAccessPermisson(Roles.CVNC.PASS_CV) : this.userService.isAccessPermisson(Roles.CVNC.PASS_DN);
        const checkApprove = this.baoCao.maLoai == Cvnc.CAP_VON ? this.userService.isAccessPermisson(Roles.CVNC.APPROVE_CV) : this.userService.isAccessPermisson(Roles.CVNC.APPROVE_DN);
        const checkExport = this.baoCao.maLoai == Cvnc.CAP_VON ? this.userService.isAccessPermisson(Roles.CVNC.EXPORT_CV) : this.userService.isAccessPermisson(Roles.CVNC.EXPORT_DN);
        const checkAccept = this.baoCao.maLoai == Cvnc.CAP_VON ? false : this.userService.isAccessPermisson(Roles.CVNC.ACCEPT_DN);
        this.status.save = Status.check('saveWHist', this.baoCao.trangThai) && isChild && checkSave;
        this.status.submit = Status.check('submit', this.baoCao.trangThai) && isChild && checkSubmit && !(!this.baoCao.id);
        this.status.pass = Status.check('pass', this.baoCao.trangThai) && isChild && checkPass;
        this.status.approve = Status.check('approve', this.baoCao.trangThai) && isChild && checkApprove;
        this.status.accept = Status.check('accept', this.baoCao.trangThai) && this.isParent && checkAccept;
        this.status.export = Status.check('export', this.baoCao.trangThai) && checkExport && (isChild || this.isParent);
