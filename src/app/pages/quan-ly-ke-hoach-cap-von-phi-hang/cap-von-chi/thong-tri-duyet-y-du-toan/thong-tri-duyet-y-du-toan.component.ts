  ...
  xoaItem(item: any) {
    this.modal.confirm({
      ...
      nzOnOk: () => {
        this.spinner.show();
        try {
          this.thongTriDuyetYCapVonService.deleteData(item.id).then((res) => {
            if (res.msg == MESSAGE.SUCCESS) {
  ...
  exportData() {
    if (this.totalRecord > 0) {
      ...
      try {
        ...
        this.thongTriDuyetYCapVonService
          .exportList(body)
          .subscribe((blob) =>
      ...
      this.modal.confirm({
        ...
        nzOnOk: async () => {
          ...
          try {
            ...
            let res = await this.thongTriDuyetYCapVonService.deleteMultiple(body);
            if (res.msg == MESSAGE.SUCCESS) {
