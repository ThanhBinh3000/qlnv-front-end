...
@Component({
  ...
  templateUrl: './thong-tin-quyet-dinh-phe-duyet-kqlcnt-scl.component.html',
  styleUrls: ['./thong-tin-quyet-dinh-phe-duyet-kqlcnt-scl.component.scss']
})
export class ThongTinQuyetDinhPheDuyetKqlcntSclComponent extends Base2Component implements OnInit {
  formData: FormGroup;
  ...
  xoa() {
    this.modal.confirm({
      ...
      nzOnOk: async () => {
        this.spinner.show();
        try {
          let res = await this.quyetdinhpheduyetKqLcntService.delete({id: this.idInput});
          if (res.msg == MESSAGE.SUCCESS) {
