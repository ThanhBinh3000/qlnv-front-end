...
@Component({
  selector: 'app-sl-gtri-hang-dtqg-xc-k-thu-tien',
  templateUrl: './sl-gtri-hang-dtqg-xc-k-thu-tien.component.html',
  styleUrls: ['./sl-gtri-hang-dtqg-xc-k-thu-tien.component.scss']
})
export class SlGtriHangDtqgXcKThuTienComponent extends Base2Component implements OnInit {
  listQuy: any[] = [
    {text: 'Quý I', value: 1},
    {text: 'Quý II', value: 2},
  ...
  async downloadExcel(id: any) {
    try {
      ...
      await this.bcBnTt108Service.ketXuat(body).then(async s => {
        ...
        saveAs(this.excelBlob, "bcbn_sl_gtri_hang_dtqg_xc_khong_thu_tien.xlsx");
      });
    } catch (e) {
      console.log(e);
    } finally {
  ...
  downloadPdf() {
    saveAs(this.pdfBlob, "bcbn_sl_gtri_hang_dtqg_xc_khong_thu_tien.pdf");
  }
}
