...
@Component({
  selector: 'app-sl-gtri-hang-dtqg-nhap',
  templateUrl: './sl-gtri-hang-dtqg-nhap.component.html',
  styleUrls: ['./sl-gtri-hang-dtqg-nhap.component.scss']
})
export class SlGtriHangDtqgNhapComponent extends Base2Component implements OnInit {
  listQuy: any[] = [
    {text: 'Quý I', value: 1},
    {text: 'Quý II', value: 2},
  ...
  async downloadExcel(id: any) {
    try {
      ...
      await this.bcBnTt108Service.ketXuat(body).then(async s => {
        ...
        saveAs(this.excelBlob, "bcbn_sl_gtri_hang_dtqg_nhap_trong_ky.xlsx");
      });
    } catch (e) {
      console.log(e);
    } finally {
  ...
  downloadPdf() {
    saveAs(this.pdfBlob, "bcbn_sl_gtri_hang_dtqg_nhap_trong_ky.pdf");
  }
}
