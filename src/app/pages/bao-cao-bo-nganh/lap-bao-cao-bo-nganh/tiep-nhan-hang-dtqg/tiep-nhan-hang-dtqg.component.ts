...
@Component({
  selector: 'app-tiep-nhan-hang-dtqg',
  templateUrl: './tiep-nhan-hang-dtqg.component.html',
  styleUrls: ['./tiep-nhan-hang-dtqg.component.scss']
})
export class TiepNhanHangDtqgComponent extends Base2Component implements OnInit {
  listQuy: any[] = [
    {text: 'Quý I', value: 1},
    {text: 'Quý II', value: 2},
  ...
  async downloadExcel(id: any) {
    try {
      ...
      await this.bcBnTt108Service.ketXuat(body).then(async s => {
        ...
        saveAs(this.excelBlob, "bcbn_tiep_nhan_hang_dtqg.xlsx");
      });
    } catch (e) {
      console.log(e);
    } finally {
  ...
  downloadPdf() {
    saveAs(this.pdfBlob, "bcbn_tiep_nhan_hang_dtqg.pdf");
  }
}
