...
export class ChiTietNhapXuatTonKhoHangDtqgComponent extends Base2Component implements OnInit {
  ...
  async search() {
    ...
    let body = {
      ...
      kyBc: this.formData.get('quy').value,
      bieuSo: this.BIEU_SO,
  ...
  downloadPdf() {
    saveAs(this.pdfBlob, "bcbn_ct_nhap_xuat_ton_kho_hang_dtqg.pdf");
  }
  ...
  async downloadExcel(id: any) {
    try {
      ...
      body.idHdr = id;
      body.typeFile = "xlsx";
      body.fileName = "bcbn_ct_nhap_xuat_ton_kho_hang_dtqg.jrxml";
      ...
      await this.bcBnTt145Service.ketXuat(body).then(async s => {
        ...
        saveAs(this.excelBlob, "bcbn_ct_nhap_xuat_ton_kho_hang_dtqg.xlsx");
      });
