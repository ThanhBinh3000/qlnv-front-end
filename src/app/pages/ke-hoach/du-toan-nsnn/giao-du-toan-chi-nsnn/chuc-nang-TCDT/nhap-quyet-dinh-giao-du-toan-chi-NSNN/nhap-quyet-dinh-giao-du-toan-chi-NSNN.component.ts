...
import { DON_VI_TIEN, GDT, LA_MA, MONEY_LIMIT, ROLE_LANH_DAO, ROLE_TRUONG_BO_PHAN, TRANG_THAI_TIM_KIEM, Utils } from 'src/app/Utility/utils';
...
export class NhapQuyetDinhGiaoDuToanChiNSNNComponent implements OnInit {
          ...
          if (data.statusCode == 0) {
            ...
            if (!this.id) {
              this.router.navigate([
                MAIN_ROUTE_KE_HOACH + '/' + MAIN_ROUTE_DU_TOAN + '/' + GIAO_DU_TOAN + '/nhap-quyet-dinh-giao-du-toan-chi-NSNN-BTC/' + data.data.id,
              ])
            }
            else {
              await this.getDetailReport();
  ...
  close() {
    // this.location.back();
    this.router.navigate([
      MAIN_ROUTE_KE_HOACH + '/' + MAIN_ROUTE_DU_TOAN + '/' + GIAO_DU_TOAN + '/tim-kiem-quyet-dinh-nhap-du-toan-chi-NSNN'
    ]);
  }
