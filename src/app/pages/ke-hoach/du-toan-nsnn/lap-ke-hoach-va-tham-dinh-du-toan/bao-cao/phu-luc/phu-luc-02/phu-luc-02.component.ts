...
import { displayNumber, DON_VI_TIEN, exchangeMoney, LA_MA, MONEY_LIMIT, sumNumber, Utils, LTD } from "src/app/Utility/utils";
...
export class ItemData {
  ...
  diaPhuongNhan: string;
  qdGiaoNvTc: string;
  luongNvThang: number;
  thanhTienNvThang: number;
  luongUocNam: number;
  thanhTienUocNam: number;
  tongCong: number;
  checked: boolean;
  isDelete: false;
}
...
export class PhuLuc02Component implements OnInit {
  ...
  // setLevel() {
  //   this.lstCtietBcao.forEach(item => {
  //     const str: string[] = item.diaPhuongNhan.split('.');
  ...
  sum(stt: string) {
    ...
    while (stt != '0') {
      ...
      this.lstCtietBcao[index] = {
        ...
        id: data.id,
        stt: data.stt,
        diaPhuongNhan: data.diaPhuongNhan,
        // level: data.level,
      }
      this.lstCtietBcao.forEach(item => {
  ...
  addLine(id: number): void {
    const item: ItemData = {
      ...
      qdGiaoNvTc: '',
      luongNvThang: null,
      thanhTienNvThang: null,
      luongUocNam: null,
      thanhTienUocNam: null,
      tongCong: null,
      checked: false,
      isDelete: false,
    };
