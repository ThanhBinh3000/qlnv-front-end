...
export class BieuMau133Component implements OnInit {
	...
	async initialization() {
		...
		if (this.status.general) {
			...
			this.scrollX = this.genFunc.setTableWidth(650, 15, BOX_NUMBER_WIDTH, 110);
		} else {
			this.scrollX = this.genFunc.setTableWidth(650, 15, BOX_NUMBER_WIDTH, 0);
		}
	...
	async save(trangThai: string, lyDoTuChoi: string) {
		...
		if (this.lstCtietBcao.some(e => e.kphiPduyetTso > MONEY_LIMIT || e.kphiThienNamTso > MONEY_LIMIT || e.kphiThienLkeTso > MONEY_LIMIT || e.kphiThienDtoanTso > MONEY_LIMIT)) {
	...
	getChiMuc(str: string): string {
		...
		switch (n) {
			...
			case 1:
				return this.genFunc.laMa(parseInt(chiSo[n - 1], 10)) + '.' + chiSo[n];
			case 2:
	...
	changeModel(id: string): void {
		this.editCache[id].data.kphiPduyetTso = this.numFunc.sum([this.editCache[id].data.kphiPduyetNsnn, this.editCache[id].data.kphiPduyetNkhac]);
		this.editCache[id].data.kphiThienNamTso = this.numFunc.sum([this.editCache[id].data.kphiThienNamNsnnUth, this.editCache[id].data.kphiThienNamKphiThien]);
		this.editCache[id].data.kphiThienLkeTso = this.numFunc.sum([this.editCache[id].data.kphiThienLkeNsnn, this.editCache[id].data.kphiThienLkeNkhac]);
		this.editCache[id].data.kphiThienDtoanTso = this.numFunc.sum([this.editCache[id].data.kphiThienDtoanNsnn, this.editCache[id].data.kphiThienDtoanNkhac]);
	}
