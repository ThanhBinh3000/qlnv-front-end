...
export class DuToanXuatNhapHangDtqgHangNamComponent implements OnInit {
	...
	async doCopy() {
		...
		let request = {
			...
			maDvi: this.maDonViTao,
			maDviTien: this.maDviTien,
			maLoaiBcao: this.maLoaiBaoCao = QLNV_KHVONPHI_TC_DTOAN_PHI_NXUAT_DTQG_THOC_GAO_HNAM,
			namHienHanh: this.namBaoCaoHienHanh,
			namBcao: this.namBaoCaoHienHanh + 1,
			soVban: null,
		};
		...
		this.quanLyVonPhiService.trinhDuyetService(request).toPromise().then(
			async data => {
				if (data.statusCode == 0) {
					...
					await this.getDetailReport();
					this.getStatusButton();
					this.router.navigateByUrl('/qlkh-von-phi/quan-ly-lap-tham-dinh-du-toan-nsnn/du-toan-xuat-nhap-hang-dtqg-hang-nam/' + this.id);
				} else {
					this.notification.error(MESSAGE.ERROR, data?.msg);
				}
			},
