...
import { divMoney, DON_VI_TIEN, MONEY_LIMIT, mulMoney, QLNV_KHVONPHI_TC_THOP_NNCAU_CHI_TX_GD3N, Utils } from 'src/app/Utility/utils';
...
export class Tonghopnhucauchithuongxuyengiaidoan3namComponent implements OnInit {
  ...
  async luu() {
    ...
    this.lstCTietBCao.filter(item => {
       ...
       if(namHhanhN > MONEY_LIMIT || tranChiN1 >MONEY_LIMIT || ncauChiN1 > MONEY_LIMIT || clechTranChiVsNcauChiN1 > MONEY_LIMIT ||
        tranChiN2 > MONEY_LIMIT || ncauChiN2 > MONEY_LIMIT || clechTranChiVsNcauChiN2 > MONEY_LIMIT || tranChiN3 > MONEY_LIMIT ||
