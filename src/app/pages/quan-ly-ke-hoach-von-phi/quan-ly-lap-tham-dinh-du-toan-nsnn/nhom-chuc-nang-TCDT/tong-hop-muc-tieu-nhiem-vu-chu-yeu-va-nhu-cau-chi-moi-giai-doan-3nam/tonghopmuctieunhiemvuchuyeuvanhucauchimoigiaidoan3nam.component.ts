...
export class Tonghopmuctieunhiemvuchuyeuvanhucauchimoigiaidoan3namComponent implements OnInit {
 ...
 callListLinhVucChi() {
  ...
  async luu() {
    ...
    this.lstCTietBCao.filter(item => {
        let nguonKphi = mulMoney(item.nguonKphi, this.donvitien);
        let tongSo = mulMoney(item.tongSo, this.donvitien);
        let chiCs = mulMoney(item.chiCs, this.donvitien);
        let chiMoi = mulMoney(item.chiMoi, this.donvitien);
        let dtuPtrien = mulMoney(item.dtuPtrien, this.donvitien);
        let dtuPtrienChiCs = mulMoney(item.dtuPtrienChiCs, this.donvitien);
        let dtuPtrienChiMoi = mulMoney(item.dtuPtrienChiMoi, this.donvitien);
      ...
      lstCTietBCaoTemp.push({
        ...
        chiMoi : chiMoi,
        dtuPtrien : dtuPtrien,
        dtuPtrienChiCs : dtuPtrienChiCs,
        dtuPtrienChiMoi : dtuPtrienChiMoi,
        chiTx : chiTx,
        chiTxChiCs : chiTxChiCs,
        chiTxChiMoi : chiTxChiMoi,
      })
    });
