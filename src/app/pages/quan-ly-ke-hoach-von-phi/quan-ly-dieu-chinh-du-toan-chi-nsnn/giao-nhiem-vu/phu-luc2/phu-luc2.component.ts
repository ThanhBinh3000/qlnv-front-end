...
export class PhuLuc2Component implements OnInit {
  ...
  async ngOnInit() {
    ...
    // this.lstCtietBcao = this.data?.lstCtietDchinh;
  ...
  async save(trangThai: string) {
    ...
    this.lstCtietBcao.forEach(item => {
      ...
      if (thienSluongKhoachDgiao > MONEY_LIMIT || thienSluongTteThien > MONEY_LIMIT ||
        thienSluongUocThien > MONEY_LIMIT || thienCong > MONEY_LIMIT || thienDinhMuc > MONEY_LIMIT || thienThanhTien > MONEY_LIMIT || kphiThieuNtruoc > MONEY_LIMIT || ncauKphi > MONEY_LIMIT
      ) {
