    ...
    try {
      ...
      if (res.msg == MESSAGE.SUCCESS) {
        if (res.data.noiDungCuuTro) {
          ...
          this.dsDiaDiem = uniqBy(res.data.dataDtl, "maDvi").filter(f => f.tenNganKho && f.maDvi && f.maDvi.startWith(this.userInfo.MA_DVI) && listBienBanLayMau.every(b => b.maDiaDiem !== f.maDvi));
          this.formData.patchValue({
            idQdGnv: res.data.id,
            soQdGnv: res.data.soBbQd,
            ngayKyQdGnv: res.data.ngayKy,
